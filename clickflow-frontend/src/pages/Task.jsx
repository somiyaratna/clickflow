...
function Task() {
    ...
    const handleSubmit = async () => {
        try {
            const submitTask = await taskSubmission(user?._id, product?.title, product.price, product?.image_url, product?.category, product?.description, commission)
            if (submitTask.message === "New daily task created" || submitTask.message === "Task count updated") {
                toast.success("Task Completed Successfully.")
                fetchProduct()
                dailyTask()
            }else if(submitTask.message === "All Tasks are Completed for today"){
                toast.warning("All Tasks are Completed for today")
            }
        } catch (error) {
            console.error(error.message);
        } finally {
    ...
    return (
        <div className="min-h-screen p-4 md:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <Button onClick={handleSubmit} width={"full"}>Submit</Button>
            </div>
